#!/bin/bash
ln -s $HOME/Documents/dotfiles/alacritty ~/.config/
ln -s $HOME/Documents/dotfiles/betterlockscreen ~/.config/
ln -s $HOME/Documents/dotfiles/dunst ~/.config/
ln -s $HOME/Documents/dotfiles/lf ~/.config/
ln -s $HOME/Documents/dotfiles/mpd ~/.config/
ln -s $HOME/Documents/dotfiles/mpv ~/.config/
ln -s $HOME/Documents/dotfiles/ncmpcpp ~/.config/
ln -s $HOME/Documents/dotfiles/neofetch ~/.config/
ln -s $HOME/Documents/dotfiles/nsxiv ~/.config/
ln -s $HOME/Documents/dotfiles/picom ~/.config/
ln -s $HOME/Documents/dotfiles/polybar ~/.config/
ln -s $HOME/Documents/dotfiles/qtile ~/.config/
ln -s $HOME/Documents/dotfiles/redshift ~/.config/
ln -s $HOME/Documents/dotfiles/rofi ~/.config/
ln -s $HOME/Documents/dotfiles/scripts ~/.config/
ln -s $HOME/Documents/dotfiles/zathura ~/.config/

ln -s $HOME/Documents/dotfiles/.profile ~/
ln -s $HOME/Documents/dotfiles/.Xresources ~/
ln -s $HOME/Documents/dotfiles/.zshrc ~/
ln -s $HOME/Documents/dotfiles/mimeapps.list ~/.config/
ln -s $HOME/Documents/dotfiles/wallpaper.png ~/.config
