...
main(){
	...
	sudo pacman -S  "$(cut -d " " -f 1 < "$(find "$HOME"/dots/misc -type f -iname "npkglist*" -exec ls -t -1 {} + | head -n 1)")" &&
	echo "installing yay" &&
	git clone "https://aur.archlinux.org/yay.git" "$HOME"/yay && cd "$HOME"/yay && makepkg -si &&
	echo "Installing AUR packages" &&
	yay -S "$(cut -d " " -f 1 < "$(find "$HOME"/dots/misc -type f -iname "fpkglist*" -exec ls -t -1 {} + | head -n 1)")" &&
